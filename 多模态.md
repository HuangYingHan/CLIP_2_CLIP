...
4. 中文CLIP是如何基于CLIP得到的（如何微调网络，如何微调数据集)
   ...
   > 采用了中文图对数据 (数据集名称 * 2)
   >
   > 采用中文encoder (chinese robot),并冻结VIT encoder,只对中文encoder进行训练, 
...
5. CLIP两大类微调方法
   ...
   2. coop微调位置
      ...
      > 原本的CLIP在zero shot任务迁移时，需要生成合适的prompt, 甚至可以外界一个transformer做一个生成专门样式的prompt的NLP任务
      >
...
7. BLIP2 
   ...
   >   1. 阶段 1：联合视觉编码器训练
   >      1. 与BLIP相同，设置三个目标函数
   >         1. ITC：正样本相似度高，负样本相似度低（==Attention mask?==）
...
8. MiniGPT4
   1. > 1. 如何通过BLIP微调而来
      ...
      >
      >    1. 第一阶段数据集：chatgpt归纳
      >       1. 采用遵循ViCUNA对话格式的prompt, 并通过控制生成句子达到一定的token数保证prompt足够丰富
      >       2. 随机选择5000个图像，查看上述方法得到的详细描述信息，手动筛选出3500张高质量的信息
      >    2. 第二阶段数据集：
...
10. LLAMA
    1. LLAMA网络架构及设计原理
       ...
       > 但是：
       >
       > 1. 采用在从embeded patch提取QKV前使用RMSNorm
       > 2. 在QK上采用旋转位置编码  (数学形式)
